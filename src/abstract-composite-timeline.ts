...
export abstract class AbstractCompositeTimeline<T extends Timeline> extends AbstractTimeline implements Timeline {
  ...
  public constructor(protected readonly timelines: ReadonlyArray<T>, length: number) {
    ...
    const register = () => {
      this.timelines.forEach(t => TimelineRunner.unregisterTimeline(t));
      TimelineRunner.registerTimeline(this);
    };

    const unregister = () => TimelineRunner.unregisterTimeline(this);
    ...
    timelines.forEach(t => TimelineRunner.unregisterTimeline(t));
    TimelineRunner.registerTimeline(this);
  }
}
